...
var validation = (function (){
	...
	var init = function(){
		...
		_setUpListners = function () { // Прослушивает все события 
	      ...
	      $('form').on('keydown', '.has-error', removeError). // удаляем красную обводку у элементов форм
	      on('reset', clearForm); // при сбросе формы удаляем также: тултипы, обводку, сообщение от сервера

	      

	    },
    	removeError = function() { // Убирает красную обводку у элементов форм
	      console.log('Красная обводка у элементов форм удалена');

	      $(this).removeClass('has-error');
	    },	    
	    clearForm = function(form) { // Очищает форму 
	    ...
	    createQtip = function (element, position) { // Создаёт тултипы
